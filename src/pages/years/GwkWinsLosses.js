...
export default function GwkWinsLosses({ data }) {
    const keys = Object.keys(data[0])
    const winOrLoss = keys[1]
  ...
  return (
    <div className='my-2'>
        <div className="flex">
          <h2 className='text-xl underline p-2'>Gameweek {winOrLoss}</h2>
          { !dropdown ? 
        ...
        { dropdown && <div className='animate-fadein overflow-x-auto'>
            <table>
              ...
              <tbody>
                <tr>
                  { data.map(player => (
                    <td key={Math.random()} className='border-4 border-secondary text-center p-2'>{player[winOrLoss]}</td>
                  ))}
                </tr>
              </tbody>
            </table>
